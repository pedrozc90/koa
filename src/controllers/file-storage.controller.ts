...
import { createPage, IFileStorage } from "../types";
import { createHash } from "../utils";

export const fetch = async (ctx: Context, next: Next) => {
	const page = Number(ctx.query.page) || 1;
	const rows = Number(ctx.query.rows) || undefined;

	const list: any[] = ["Dockerfile", "docker-compose.yml", "package.json", "README.md"].map((filename) => {
		try {
	...
	ctx.status = 200;
	ctx.body = createPage<IFileStorage>({ page, rows, count: list.length, list });
};
